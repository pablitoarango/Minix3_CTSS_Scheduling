...
void user_dmp()
{
  /* Proc table dump */
  register struct proc *rp;
  static struct proc *oldrp = BEG_USER_ADDR;
  int r;
  ...
  char user_process[PROC_NAME_LEN] = "loop"; // can be changed to your program name
  for (rp = oldrp; rp < END_PROC_ADDR; rp++) {
    if (isemptyp(rp))
      continue;
    if(strcmp (user_process,rp->p_name) == 0)
    {
      printf("Loop: End-point: %d, Priority: %d; Quantum: %d;\n", rp->p_endpoint, rp->p_priority, rp->p_quantum_size_ms);
    }
    }
}
